...
4. **Cut** — `audio/AudioTrimmer` seeks to the previous sync sample, walks forward to the exact
   ...
   **With a fade — or a source MediaMuxer cannot hold, like MP3, WAV or FLAC** — that shortcut is
   ...
   playback stays a plain `MediaPlayer.start()`. The editor previews fades by riding the preview
   player's volume, so you hear them before anything is written.
...
```
audio/    AudioFetcher, LocalAudioImporter, AudioTrimmer, Waveform, SoundPlayer
